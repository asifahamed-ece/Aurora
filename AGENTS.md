...
- `include/buttons.h` & `src/block1/buttons.cpp` - Two-button handler (Warm-Touch GPIO0 + mode/WiFi GPIO2)
...
- **Mode / WiFi Button**: GPIO2 (`AURORA_BTN_MODE_PIN`). Short press cycles the 6 display screens (Deskmate → Clock & Date → Daily Quote → Scan WiFi QR → Scan Dashboard QR → Heartbeat Keepsake); 3-second hold toggles the WiFi softAP.
